...
### 나의 생각
해당 문제는 해결법이 여러개 생각났었는데 <br>
<ul>
        <li>해당코드에 해당하는 스트림 활용법</li>
        <li>for문을 역순으로 돌려 배열을 선언</li>
        <li>이중 for문을 이용한 버블정렬</li>
</ul>
이정도 생각이 났다. 이중엔 솔직히 나에겐 익숙한 방법은 for문을 역순으로 해서 배열을 선언 하는 방법이였는데 새로 알게된 스트림이 생각나서 활용해보았다. 
stream 굿! GOOD!
